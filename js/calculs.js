...
nst1++
     if (nst1 > 2) {nst1 = 1}
     if (nst1 == 1) {form.nslat1.value = "Nord"}
     if (nst1 == 2) {form.nslat1.value = "Sud"}
}

function ns2(form) {
nst2++
     if (nst2 > 2) {nst2 = 1}
     if (nst2 == 1) {form.nslon1.value = "Est"}
     if (nst2 == 2) {form.nslon1.value = "Ouest"}
}
...
document.calc.joursem.value = joursemain;
}
...
console.log(document.calc);
//--------------- Données pour circonstances locales

latdeg = document.calc.latdeg.value-0
latmin = document.calc.latmin.value-0
londeg = document.calc.londeg.value-0
lonmin = document.calc.lonmin.value-0
lev = document.calc.lev.value-0
mer = document.calc.mer.value-0
cou = document.calc.cou.value-0
...
document.calc.jourjulien.value = jj

joursem()

//--------------- Temps mesuré en siècle juliens de 36525 jours des éphémérides
...
if (annee < 1620 || annee >= 2014) 
       {
       T3=(annee - 2000) / 100
       if (annee > -391 &&  annee <= +948){dtAm = + 2177 + (497 * T3) + (44.1 * T3 * T3); dtA = dtAm} //pour les années de -391 à +948
       if (annee > +948 && annee < +1620){dtAm = + 102 + (102 * T3) + (25.3 * T3 * T3); dtA = dtAm} //pour les années de +948 à +1600
       }
else {var dtA = eval('deltaT_' +  annee)}
if (annee >= 2014)     //correspond à l'année supérieure à la dernière année "estimée" indiquée dans le fichier DeltaT.dat
       {
       T3=(annee - 2000) / 100
       dtAm = + 102 + (102 * T3) + (25.3 * T3 * T3) + 0.56 *(annee - 2100); dtA = dtAm 
       }
document.calc.dt.value = tronque(dtA * 100) /100;

coeff = 86400
jj2 = jj + (dtA / coeff);
document.calc.jourjulien2.value = jj2
JD = document.calc.jourjulien2.value-0;
...
document.calc.utday.value = JJ;
document.calc.utmonth.value = MM;
document.calc.utyear.value = AA;
document.calc.uth.value = Hre;
document.calc.utm.value = Mn;
document.calc.uts.value = floor(Sec);
...
Jts = floor(jj + 0.5) - 0.5; // jour julien à 0h00 UT
Tts = (Jts - 2415020.0) / 36525;
ts0 = 0.276919398 + 100.0021359 * Tts + 0.000001075 * Tts * Tts; 
ts1 = (ts0 - floor(ts0)) * 24 // temps sidéral à Greenwich à 0h00 UT
...
tsh = floor(ts)
tsm = floor((ts - floor(ts)) * 60)
tss = (((ts - floor(ts)) * 60) - tsm) * 60
document.calc.ts.value = jh(tsh) + "h" + zero(tsm) + "m" + zero(floor(tss)) + "s";
...
document.calc.constell.value = constell;
document.calc.longsol.value = floor(long1) + "°" + zero(floor(d1)) + "'" + zero(floor(d2 * 100 + 0.5) / 100) + "''";
document.calc.latsol.value = "0";
document.calc.rayonsol.value = floor(R * 10000000 + 0.5) / 10000000 + " UA";
...
eqT = eqT * r2d
document.calc.eqT.value = tronque((eqT * 60 / 15) * 10000) /10000;
...
document.calc.alphasol.value = floor(asc) + "h" + zero(floor(d1)) + "m" + zero(floor(d2 * 100 + 0.5) / 100) + "s";
ADSoleil = asc * 15 / r2d
...
document.calc.deltasol.value = signe + floor(d) + "°" + zero(floor(d1)) + "'" + zero(floor(d2 * 100 + 0.5) / 100) + "''";
ascSoleil = asc
declinSoleil = declin
...
document.calc.diametremerc.value = floor(diametre / distance_terre * 100 + 0.5) / 100 + "''";
...
document.calc.elongatmerc.value = floor(elongation * 10 + 0.5) / 10 + "° " + elongation_planete;
...
if (elongationMerc > 20 && elongationMerc < 50){
  ...
  if (elongation_planeteMerc == "Est") {quandMerc = "au crépuscule"}}

document.calc.visibmerc.value = commentMerc + " " + quandMerc
...
document.calc.longmerc.value = floor(l1) + "°" + zero(floor(d1)) + "'" + zero(floor(d2 * 100 + 0.5) / 100) + "''";
...
document.calc.latmerc.value = signe + floor(b1) + "°" + zero(floor(d1)) + "'" + zero(floor(d2 * 100 + 0.5) / 100) + "''";
document.calc.rayonmerc.value = floor(r * 100000 + 0.5) / 100000 + " UA"
document.calc.distancemerc.value = floor(distance_terre * 100000 + 0.5) / 100000 + " UA"
...
document.calc.alphamerc.value = floor(asc) + "h" + zero(floor(d1)) + "m" + zero(floor(d2 * 100 + 0.5) / 100) + "s";
ADMerc = asc * 15 / r2d
...
if (declin < 0) {signe = "-"; DecMerc = - d / r2d}
else {signe = "+"; DecMerc = + d / r2d}

document.calc.deltamerc.value = signe + floor(d) + "°" + zero(floor(d1)) + "'" + zero(floor(d2 * 100 + 0.5) / 100) + "''";
ascMerc = asc
declinMerc = declin
...
document.calc.magmerc.value = magnitude;
document.calc.phasemerc.value = floor(phase * 10) / 10 + "%";
...
document.calc.diametreven.value = floor(diametre / distance_terre * 100 + 0.5) / 100 + "''";
...
document.calc.elongatven.value = floor(elongation * 10 + 0.5) / 10 + "° " + elongation_planete;
...
document.calc.visibven.value = commentVen + " " + quandVen
...
document.calc.longven.value = floor(l1) + "°" + zero(floor(d1)) + "'" + zero(floor(d2 * 100 + 0.5) / 100) + "''";
...
document.calc.latven.value = signe + floor(b1) + "°" + zero(floor(d1)) + "'" + zero(floor(d2 * 100 + 0.5) / 100) + "''";
document.calc.rayonven.value = floor(r * 100000 + 0.5) / 100000 + " UA"
document.calc.distanceven.value = floor(distance_terre * 100000 + 0.5) / 100000 + " UA"
...
document.calc.alphaven.value = floor(asc) + "h" + zero(floor(d1)) + "m" + zero(floor(d2 * 100 + 0.5) / 100) + "s";
ADVen = asc * 15 / r2d
...
document.calc.deltaven.value = signe + floor(d) + "°" + zero(floor(d1)) + "'" + zero(floor(d2 * 100 + 0.5) / 100) + "''";
ascVen = asc
declinVen = declin
...
document.calc.magven.value = magnitude;
document.calc.phaseven.value = floor(phase * 10) / 10 + "%";
...
document.calc.diametremars.value = floor(diametre / distance_terre * 100 + 0.5) / 100 + "''";
...
document.calc.elongatmars.value = floor(elongation * 10 + 0.5) / 10 + "° " + elongation_planete;
...
document.calc.visibmars.value = commentMars + " " + quandMars
...
document.calc.longmars.value = floor(l1) + "°" + zero(floor(d1)) + "'" + zero(floor(d2 * 100 + 0.5) / 100) + "''";
...
document.calc.latmars.value = signe + floor(b1) + "°" + zero(floor(d1)) + "'" + zero(floor(d2 * 100 + 0.5) / 100) + "''";
document.calc.rayonmars.value = floor(r * 100000 + 0.5) / 100000 + " UA"
document.calc.distancemars.value = floor(distance_terre * 100000 + 0.5) / 100000 + " UA"
...
document.calc.alphamars.value = floor(asc) + "h" + zero(floor(d1)) + "m" + zero(floor(d2 * 100 + 0.5) / 100) + "s";
ADMars = asc * 15 / r2d
...
document.calc.deltamars.value = signe + floor(d) + "°" + zero(floor(d1)) + "'" + zero(floor(d2 * 100 + 0.5) / 100) + "''";
ascMars = asc
declinMars = declin
...
document.calc.magmars.value = magnitude;
document.calc.phasemars.value = floor(phase * 10) / 10 + "%";
...
document.calc.visibLune.value = commentLune + "" + quandLune + "" + da_Lune_Merc + "" + da_Lune_Ven + "" + da_Lune_Mars + "" + da_Lune_Jup + "" + da_Lune_Sat + "" + da_Lune_Uran + "" + da_Lune_Nept + "" + da_Lune_Pluton;
...
// note : si l'écart entre l'instant de la phase et le moment du calcul est supérieur à 15 jours, le calcul est effectué en se décalant de 15 jours afin d'afficher aussi bien les phases précédentes que les phases suivantes par rapport à la date du jour
